...
/**
 * The three documents, in one place because two surfaces need them.
 *
 * The Legal screen shows them, and so does the sign-in sheet — which cannot
 * navigate to that screen at all, since the layout renders sign-in *instead
 * of* the navigator when nobody is signed in. Two copies of a privacy policy
 * would drift, and the half that drifts is the half nobody is reading.
 *
 * Paragraphs are separate strings so they can be spaced properly rather than
 * run together by newlines. They describe what the app actually does; a
 * shipped product still needs them reviewed by a lawyer, and both surfaces say
