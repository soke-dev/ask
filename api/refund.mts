...
console.log(`  ${job.amountUsdc} USDC held · status ${job.status} · asker ${job.asker}`);

// The contract compares against block time, so that is what to wait on.
for (;;) {
  const block = await pc.getBlock();
  const left = job.deadline - Number(block.timestamp);
  if (left <= 0) break;
  console.log(`  waiting ${left}s for the deadline (chain time ${Number(block.timestamp)})`);
  await new Promise((r) => setTimeout(r, Math.min(left, 60) * 1000 + 3000));
}

const result = await relayRefund(jobId);
console.log('  refunded · tx', result.txHash);
console.log('  gas paid by the relayer:', result.gasEth, 'ETH');

const after = await readJob(jobId);
console.log('  chain now says:', after?.status);

// Record it so the ledger explains where the money went.
...
if (q[0]) {
  ...
  console.log('  ledger : refund recorded');
}
